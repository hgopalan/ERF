...
# in the band, and the number of embers that landed on the footprint.
...
VARIANTS="noib ib noib_spotting"

printf "%-14s %3s %6s %6s %7s %7s %7s %6s %9s %9s %9s %7s\n" variant id x y burned t_first t_last resid peak_kWm HL_mean HL_max embers
printf "%-14s %3s %6s %6s %7s %7s %7s %6s %9s %9s %9s %7s\n" -------------- --- ------ ------ ------- ------- ------- ------ --------- --------- --------- -------
...
for v in $VARIANTS; do
    ...
    while IFS=, read -r t id x y h foot wall wf t0 t1 res pk hlm hlx emb; do
        printf "%-14s %3s %6.0f %6.0f %7.2f %7.0f %7.0f %6.0f %9.1f %9.3f %9.3f %7s\n" "$v" "$id" "$x" "$y" "$wf" "$t0" "$t1" "$res" "$pk" "$hlm" "$hlx" "$emb"
    done
done
