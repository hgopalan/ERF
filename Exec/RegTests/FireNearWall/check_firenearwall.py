#!/usr/bin/env python3
"""Sanity checks on the fire output of the FireNearWall tests.

    python3 check_firenearwall.py [plt_fire_NNNNN] [--stats fire_stats.csv]
...
and the script exits 1 if any fails, so it can follow the run in CTest:
    ...
    erf_exec inputs_... && python3 check_firenearwall.py
...
def main():
    ...
    print(f"FireNearWall: {pf} at t = {t:.3f} s")
    ...
    print(f"FireNearWall: {len(results) - n_fail}/{len(results)} checks passed")
