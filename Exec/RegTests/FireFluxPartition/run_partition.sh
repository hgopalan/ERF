...
# two as the weaker heating changes the wind.
...
VARIANTS="default legacy cfbm cfbm_wet cfbm_nolatent legacy_2way cfbm_2way"
...
printf "%-16s %6s %8s %8s %12s %12s %7s\n" variant M_f f_dry steps sens_max_W latent_max_W cells
printf "%-16s %6s %8s %8s %12s %12s %7s\n" ---------------- ------ -------- -------- ------------ ------------ -------
for v in $VARIANTS; do
    printf "%-16s %6s %8s %8s %12s %12s %7s\n" "$v" "$(mf $v)" "$(fac $v)" "$(sens $v | wc -l | tr -d ' ')" \
        "$(sens $v | tail -1 | cut -c1-12)" "$(lat $v | tail -1 | cut -c1-12)" "$(cells $v)"
done
echo
