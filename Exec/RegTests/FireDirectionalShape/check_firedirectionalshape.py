...
"""FireDirectionalShape: a point ignition in a uniform wind, against Rothermel.
    ...
    python3 check_firedirectionalshape.py [isotropic projection projection_key ellipse ellipse_anderson wrf]
...
wrf               erf.fire.directional_wind_coupling = "wrf": phi_w evaluated
                  from the raw wind speed U (not projected), then the whole
                  wind/slope factor scaled by cos(theta) to the front normal,
                  F(theta) = R0 (1 + phi_w(U) max(cos theta, 0)) -- matching
                  WRF-Fire's fire_ros (module_fr_fire_phys.F). Linear in
                  cos(theta), so (like the ellipse) it is its own Wulff shape
...
def main():
    ...
    for v in (sys.argv[1:] or list(KIND)):
        ...
        for name in measured:
            ...
            if kind in ("ell", "ella", "wrf") and name.startswith("head"):
