...
= "advective"` (still `directional_shape = "projection"`), against Rothermel's
head rate and the exact solution of each formulation's own equation.
...
| `advective` | `directional_wind_coupling = "advective"`: R(n) = R0 (1 + phi_w(U) max(cos theta, 0)) |
...
`directional_wind_coupling = "advective"` fixes the same non-convexity a
different way, at the level of the rate-of-spread model rather than by
imposing a shape: instead of projecting the wind onto the front normal and
then exponentiating (R0 (1 + phi_w((U n_x)))), it exponentiates phi_w from the
raw wind speed once and only afterward scales the whole wind/slope factor by
cos(theta), R(n) = R0 (1 + phi_w(U) max(cos theta, 0)) -- matching WRF-Fire's
fire_ros (module_fr_fire_phys.F). Since n |grad phi| = grad phi, this R(n)
turns the level set into pure advection by a wind-aligned velocity plus
isotropic growth at R0 (the reduction the name refers to; the same one behind
the original level-set fire-spread formulation, Mandel, Beezley and Kochanski
2011). Being linear in cos(theta), it is also the support function of a
stadium (a disc of radius R0 swept along the wind vector, since the clamp at
...
this head/back/flank rate," `"advective"` asks "what is the model's own
oblique rate" -- the two happen to agree here because Rothermel's back and
flank rates are both R0, but they would differ for a model whose off-axis
rate is not simply the no-wind rate.
...
| `advective` | 0.24849 (-0.38 %) | 0.02396 | 0.02446 (+1.77 %) |

Back and flanks match the exact rates (R0 = 0.02404 m/s, and b / LW for
`ellipse_anderson`) to 0.7 %, except `advective`'s flanks at 1.77 % (front 0.44
