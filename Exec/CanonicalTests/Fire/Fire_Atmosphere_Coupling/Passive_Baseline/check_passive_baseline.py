#!/usr/bin/env python3
"""Sanity checks on the fire output of the Passive_Baseline tests.

    python3 check_passive_baseline.py [plt_fire_NNNNN] [--stats fire_stats.csv]
...
and the script exits 1 if any fails, so it can follow the run in CTest:
    ...
    erf_exec inputs_... && python3 check_passive_baseline.py
...
def main():
    ...
    print(f"Passive_Baseline: {pf} at t = {t:.3f} s")
    ...
    print(f"Passive_Baseline: {len(results) - n_fail}/{len(results)} checks passed")
