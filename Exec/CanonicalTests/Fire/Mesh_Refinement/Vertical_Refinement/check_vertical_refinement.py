#!/usr/bin/env python3
"""Sanity checks on the fire output of the Vertical_Refinement tests.

    python3 check_vertical_refinement.py [plt_fire_NNNNN] [--stats fire_stats.csv]
...
and the script exits 1 if any fails, so it can follow the run in CTest:
    ...
    erf_exec inputs_... && python3 check_vertical_refinement.py
...
def main():
    ...
    print(f"Vertical_Refinement: {pf} at t = {t:.3f} s")
    ...
    print(f"Vertical_Refinement: {len(results) - n_fail}/{len(results)} checks passed")
