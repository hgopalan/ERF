#!/usr/bin/env python3
"""Sanity checks on the fire output of the Level_Set_Advection tests.

    python3 check_level_set_advection.py [plt_fire_NNNNN] [--stats fire_stats.csv]
...
and the script exits 1 if any fails, so it can follow the run in CTest:
    ...
    erf_exec inputs_... && python3 check_level_set_advection.py
...
def main():
    ...
    print(f"Level_Set_Advection: {pf} at t = {t:.3f} s")
    ...
    print(f"Level_Set_Advection: {len(results) - n_fail}/{len(results)} checks passed")
