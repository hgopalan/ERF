#!/usr/bin/env python3
"""Sanity checks on the fire output of the Fuel_Moisture_Sensitivity tests.

    python3 check_fuel_moisture_sensitivity.py [plt_fire_NNNNN] [--stats fire_stats.csv]
...
and the script exits 1 if any fails, so it can follow the run in CTest:
    ...
    erf_exec inputs_... && python3 check_fuel_moisture_sensitivity.py
...
def main():
    ...
    print(f"Fuel_Moisture_Sensitivity: {pf} at t = {t:.3f} s")
    ...
    print(f"Fuel_Moisture_Sensitivity: {len(results) - n_fail}/{len(results)} checks passed")
