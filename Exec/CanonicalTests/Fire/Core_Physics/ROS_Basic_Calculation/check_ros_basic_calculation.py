#!/usr/bin/env python3
"""Sanity checks on the fire output of the ROS_Basic_Calculation tests.

    python3 check_ros_basic_calculation.py [plt_fire_NNNNN] [--stats fire_stats.csv]
...
and the script exits 1 if any fails, so it can follow the run in CTest:
    ...
    erf_exec inputs_... && python3 check_ros_basic_calculation.py
...
def main():
    ...
    print(f"ROS_Basic_Calculation: {pf} at t = {t:.3f} s")
    ...
    print(f"ROS_Basic_Calculation: {len(results) - n_fail}/{len(results)} checks passed")
