#!/usr/bin/env python3
"""Sanity checks on the fire output of the Wind_Speed_Variation tests.

    python3 check_wind_speed_variation.py [plt_fire_NNNNN] [--stats fire_stats.csv]
...
and the script exits 1 if any fails, so it can follow the run in CTest:
    ...
    erf_exec inputs_... && python3 check_wind_speed_variation.py
...
def main():
    ...
    print(f"Wind_Speed_Variation: {pf} at t = {t:.3f} s")
    ...
    print(f"Wind_Speed_Variation: {len(results) - n_fail}/{len(results)} checks passed")
