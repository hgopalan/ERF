#!/usr/bin/env python3
"""Sanity checks on the fire output of the Wind_Adjustment_Factor tests.

    python3 check_wind_adjustment_factor.py [plt_fire_NNNNN] [--stats fire_stats.csv]
...
and the script exits 1 if any fails, so it can follow the run in CTest:
    ...
    erf_exec inputs_... && python3 check_wind_adjustment_factor.py
...
def main():
    ...
    print(f"Wind_Adjustment_Factor: {pf} at t = {t:.3f} s")
    ...
    print(f"Wind_Adjustment_Factor: {len(results) - n_fail}/{len(results)} checks passed")
