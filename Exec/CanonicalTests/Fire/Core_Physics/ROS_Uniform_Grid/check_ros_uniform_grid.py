#!/usr/bin/env python3
"""Sanity checks on the fire output of the ROS_Uniform_Grid tests.

    python3 check_ros_uniform_grid.py [plt_fire_NNNNN] [--stats fire_stats.csv]
...
and the script exits 1 if any fails, so it can follow the run in CTest:
    ...
    erf_exec inputs_... && python3 check_ros_uniform_grid.py
...
def main():
    ...
    print(f"ROS_Uniform_Grid: {pf} at t = {t:.3f} s")
    ...
    print(f"ROS_Uniform_Grid: {len(results) - n_fail}/{len(results)} checks passed")
