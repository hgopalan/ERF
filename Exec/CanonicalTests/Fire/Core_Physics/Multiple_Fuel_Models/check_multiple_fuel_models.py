#!/usr/bin/env python3
"""Sanity checks on the fire output of the Multiple_Fuel_Models tests.

    python3 check_multiple_fuel_models.py [plt_fire_NNNNN] [--stats fire_stats.csv]
...
and the script exits 1 if any fails, so it can follow the run in CTest:
    ...
    erf_exec inputs_... && python3 check_multiple_fuel_models.py
...
def main():
    ...
    print(f"Multiple_Fuel_Models: {pf} at t = {t:.3f} s")
    ...
    print(f"Multiple_Fuel_Models: {len(results) - n_fail}/{len(results)} checks passed")
