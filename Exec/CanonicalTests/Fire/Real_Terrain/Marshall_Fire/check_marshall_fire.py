#!/usr/bin/env python3
"""Sanity checks on the fire output of the Marshall_Fire tests.

    python3 check_marshall_fire.py [plt_fire_NNNNN] [--stats fire_stats.csv]
...
and the script exits 1 if any fails, so it can follow the run in CTest:
    ...
    erf_exec inputs_... && python3 check_marshall_fire.py
...
def main():
    ...
    print(f"Marshall_Fire: {pf} at t = {t:.3f} s")
    ...
    print(f"Marshall_Fire: {len(results) - n_fail}/{len(results)} checks passed")
