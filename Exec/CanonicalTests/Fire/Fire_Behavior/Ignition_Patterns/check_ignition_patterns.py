#!/usr/bin/env python3
"""Sanity checks on the fire output of the Ignition_Patterns tests.

    python3 check_ignition_patterns.py [plt_fire_NNNNN] [--stats fire_stats.csv]
...
and the script exits 1 if any fails, so it can follow the run in CTest:
    ...
    erf_exec inputs_... && python3 check_ignition_patterns.py
...
def main():
    ...
    print(f"Ignition_Patterns: {pf} at t = {t:.3f} s")
    ...
    print(f"Ignition_Patterns: {len(results) - n_fail}/{len(results)} checks passed")
