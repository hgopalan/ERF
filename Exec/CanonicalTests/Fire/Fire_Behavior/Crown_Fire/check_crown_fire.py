#!/usr/bin/env python3
"""Sanity checks on the fire output of the Crown_Fire tests.
...
and the script exits 1 if any fails, so it can follow the run in CTest:
    ...
    erf_exec inputs_... && python3 check_crown_fire.py
...
def main():
    ...
    print(f"Crown_Fire: {pf} at t = {t:.3f} s")
    ...
    print(f"Crown_Fire: {len(results) - n_fail}/{len(results)} checks passed")
