#!/usr/bin/env python3
"""Sanity checks on the fire output of the Spatial_Fuel tests.

    python3 check_spatial_fuel.py [plt_fire_NNNNN] [--stats fire_stats.csv]
...
and the script exits 1 if any fails, so it can follow the run in CTest:
    ...
    erf_exec inputs_... && python3 check_spatial_fuel.py
...
def main():
    ...
    print(f"Spatial_Fuel: {pf} at t = {t:.3f} s")
    ...
    print(f"Spatial_Fuel: {len(results) - n_fail}/{len(results)} checks passed")
