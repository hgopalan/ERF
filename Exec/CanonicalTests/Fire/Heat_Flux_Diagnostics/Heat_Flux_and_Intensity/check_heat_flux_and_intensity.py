#!/usr/bin/env python3
"""Sanity checks on the fire output of the Heat_Flux_and_Intensity tests.

    python3 check_heat_flux_and_intensity.py [plt_fire_NNNNN] [--stats fire_stats.csv]
...
and the script exits 1 if any fails, so it can follow the run in CTest:
    ...
    erf_exec inputs_... && python3 check_heat_flux_and_intensity.py
...
def main():
    ...
    print(f"Heat_Flux_and_Intensity: {pf} at t = {t:.3f} s")
    ...
    print(f"Heat_Flux_and_Intensity: {len(results) - n_fail}/{len(results)} checks passed")
