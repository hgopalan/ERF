#!/usr/bin/env python3
"""Sanity checks on the fire output of the Flame_Temperature tests.

    python3 check_flame_temperature.py [plt_fire_NNNNN] [--stats fire_stats.csv]
...
and the script exits 1 if any fails, so it can follow the run in CTest:
    ...
    erf_exec inputs_... && python3 check_flame_temperature.py
...
def main():
    ...
    print(f"Flame_Temperature: {pf} at t = {t:.3f} s")
    ...
    print(f"Flame_Temperature: {len(results) - n_fail}/{len(results)} checks passed")
