                           ...
                           bool                      ucm_debug)
{
    ...
    if (!f_urb_atm.ok() || !H_bldg_mean_atm.ok() || !H_bldg_std_atm.ok() ||
        ...
        !H_road_atm.ok() || !H_wall_atm.ok() || !H_roof_atm.ok()) {
        if (ParallelDescriptor::IOProcessor()) {
    ...
    // Build 9-component slab MultiFab (Phase 2.7: f_urb, H_bldg_mean, H_bldg_std,
    // lambda_p, lambda_f, H_atm, H_road_atm, H_wall_atm, H_roof_atm)
    ...
    // Build component names vector (Phase 2.7: 9 components)
