...
| 5 | 5.1a | **View-factor precomputation (pure geometry)** | Hottel crossed-string per-cell view factors (F_wall_sky, F_wall_wall, F_wall_road, F_road_sky, F_road_wall, F_roof_sky) computed once per run; static-bool guard; no SEB integration | ✅ COMPLETE | #<PR> + branch-local (max_step=1 + MOST activation for unit test) |
