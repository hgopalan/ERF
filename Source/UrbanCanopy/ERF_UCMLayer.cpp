    ...
    if (!banner_23_printed && m_params.ucm_debug) {
        ...
        if (amrex::ParallelDescriptor::IOProcessor()) {
            amrex::Print() << "\n[UCM][2.3][BANNER] Phase 2.3 facet-split fluxes and AH active:\n"
                           ...
                           << " max=" << plan_max << "\n"
                           << "  H_road min="  << hr_min
                           << " max="     << hr_max << " W/m^2 (per road area)\n"
                           << "  H_wall min="  << hw_min
                           << " max="     << hw_max << " W/m^2 (per wall area, diagnostic only)\n"
                           << "  H_roof min="  << hf_min
                           << " max="     << hf_max << " W/m^2 (per roof area, incl. AH)\n"
                           << "  AH min="      << ah_min
                           << " max="     << ah_max << " W/m^2 (per plan area)\n"
                           << "  Lumped-to-ATM invariant:\n"
                           << "    H_sensible = (1-lam_p)*H_road + lam_p*(H_roof - AH) + AH\n"
                           << "    (wall enters canyon-air budget in Phase 2.4+, NOT this sum)\n\n";
        }
    }
