                      ...
                      amrex::Real depth_threshold)
{
    for (amrex::MFIter mfi(lng_pool_mask, amrex::TilingIfNotGPU()); mfi.isValid(); ++mfi)
    {
        ...
        const auto& depth_fab = lng_pool_depth[mfi];
        auto& mask_fab = lng_pool_mask[mfi];
        
        amrex::ParallelFor(bx, [=] AMREX_GPU_DEVICE(int i, int j, int k) {
            mask_fab(i, j, k) = (depth_fab(i, j, k) > depth_threshold) ? 1.0 : 0.0;
        });
    }
                         ...
                         amrex::Real dt)
{
    ...
    amrex::Real cell_area_m2 = dx[0] * dx[1];
    amrex::Real pool_radius = std::sqrt(pool_area_m2 / M_PI);
    
    // Height increment per active cell
    ...
    {
        const auto& bx = mfi.tilebox();
        auto& depth_fab = lng_pool_depth[mfi];
                                    ...
                                    bool lng_debug)
{
    ...
    for (amrex::MFIter mfi(lng_pool_depth, amrex::TilingIfNotGPU()); mfi.isValid(); ++mfi)
    {
        const auto& bx = mfi.tilebox();
        auto& depth_fab = lng_pool_depth[mfi];
        const auto& evap_fab = lng_evap_flux[mfi];
        
        amrex::ParallelFor(bx, [=] AMREX_GPU_DEVICE(int i, int j, int k) {
            amrex::Real F_evap = evap_fab(i, j, k);
            amrex::Real dh_evap = (F_evap / rho_LNG) * dt;
            depth_fab(i, j, k) = std::max(0.0, depth_fab(i, j, k) - dh_evap);
        });
    }
                               ...
                               amrex::Real rho_LNG)
{
    ...
    // Mass = sum(depth) * rho_LNG * cell_area
    amrex::Real pool_mass = depth_sum * rho_LNG * cell_area_m2;
                               ...
                               const amrex::Geometry& geom_lng)
{
    ...
    // Sum all mask values (count cells)
    amrex::Real mask_sum = lng_pool_mask.sum(0);
    
    // Area = count * cell_area
    amrex::Real pool_area = mask_sum * cell_area_m2;
