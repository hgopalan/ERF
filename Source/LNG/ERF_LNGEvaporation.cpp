                            ...
                            bool lng_debug)
{
    ...
    for (amrex::MFIter mfi(lng_evap_flux, amrex::TilingIfNotGPU()); mfi.isValid(); ++mfi)
    {
        ...
        // Get Array4 views — cheap, copyable, and GPU-safe
        const auto mask_arr   = lng_pool_mask[mfi].array();
        const auto ustar_arr  = lng_ustar[mfi].array();
        auto       evap_arr   = lng_evap_flux[mfi].array();
        auto       latent_arr = lng_latent_flux[mfi].array();

        // Parallel for loop over 2D box
        amrex::ParallelFor(bx, [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
            if (mask_arr(i, j, k) > 0.5) {
                // Pool cell: compute evaporation
                amrex::Real u_star   = ustar_arr(i, j, k);
                ...
                evap_arr(i, j, k)   = F_evap;
                latent_arr(i, j, k) = Q_latent;
            } else {
                // No pool: zero flux
                evap_arr(i, j, k)   = 0.0;
                latent_arr(i, j, k) = 0.0;
            }
        });
    }
