                                   ...
                                   const DistributionMapping& dm_in)
{
    ...
    {
        // We only decompose in z if max_grid_size_z indicates we should, and then only
        // if we are allowed to split boxes in z at all (amr.no_box_split_dir = 2, the ERF
        // default, forbids that), and never when the vertical implicit diffusion solve is
        // on: it solves one tridiagonal per column and takes each box's z extent as the
        ...
        bool decompose_in_z = (max_grid_size[0][2] < domain.length(2)) &&
                              (no_box_split_dir != 2) && !implicit_column_solve;
