    ...
    int             step)
{
    ...
    for (MFIter mfi(cc_source, TilingIfNotGPU()); mfi.isValid(); ++mfi) {
        ...
        auto src  = cc_source.array(mfi, species_comp);
        ...
        ParallelFor(bx0, [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept {
            ...
            Real dz_cell = (k < khi)
                ...
                : dz;
            // species_flux [kg/m2/s] = ef * Q [W/m2] / hoc [J/kg]
            Real species_flux = ef * amrex::max(heat(i,j,k), 0.0_rt) / hoc;
            // inject as volumetric source [kg/m3/s] = flux / dz
            src(i, j, k) += species_flux / dz_cell;
        });
    }

    if (fire_debug) {
        Real src_max  = cc_source.max(species_comp);
        Real heat_max = fire_heat_atm.max(0);
        Print() << "[FIRE DEBUG] " << species_name << ": step=" << step
                << " src_max=" << src_max << " kg/m3/s"
                << " heat_max=" << heat_max << " W/m2\n";
    }
}
